...
export default function Index({ posts, tag, pagination, page }: Props) {
  const url = `/exodus/tags/${tag.name}` + (page ? `/${page}` : "");
...
export const getStaticProps: GetStaticProps = async ({ params }) => {
  ...
  const posts = listExodusPostContent(
  ...
  const pagination = {
    ...
    pages: Math.ceil(countExodusPosts(slug) / config.posts_per_page),
  };
  const props: {
    posts: PostContent[];
    tag: TagContent;
...
export const getStaticPaths: GetStaticPaths = async () => {
  const paths = listTags().flatMap((tag) => {
    const pages = Math.ceil(countExodusPosts(tag.slug) / config.posts_per_page);
